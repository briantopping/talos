...
// mdLastResortSettled reports whether MD last-resort assembly has finished trying.
//
// An ABSENT resource is reported as settled: the controller does not run in every
// mode, and a reader must not block forever on an answer nobody is going to give.
// It publishes "unsettled" as its very first action precisely so that this window
...
func (ctrl *AcquireController) stateDisk(ctx context.Context, r controller.Runtime, logger *zap.Logger) (stateMachineFunc, config.Provider, error) {
	...
	case stateVolumeStatus.TypedSpec().Phase == block.VolumePhaseMissing:
		// STATE is missing -- but "missing" and "not assembled yet" are different
		// answers, and only one of them is safe to act on. A node booting on the
		// surviving disk of a mirrored system disk reads STATE as missing for the
		// whole last-resort grace period, and treating that as final makes it decide
		// it has no configuration at all: it drops into maintenance mode, and never
		// looks again once the array does appear.
