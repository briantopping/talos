...
// MDLastResortStatus reports whether last-resort assembly of degraded MD arrays
// has finished trying.
//
// It exists so that "this volume is missing" can be told apart from "the array
// which would provide it has not been assembled yet". Those are different answers
// and only one of them is safe to act on: a node booting on the surviving disk of
// a mirror reads STATE as missing for the whole grace period, and anything which
// treats that as final concludes the machine has no configuration.
...
type MDLastResortStatusSpec struct {
	// Settled reports that no further arrays are expected to appear: either there
	// were none inactive, or the grace has elapsed and they have been force-run.
