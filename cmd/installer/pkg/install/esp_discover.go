...
func DiscoverMirroredESP(probeOptions ...blkid.ProbeOption) (string, error) {
	sysBlock, err := os.ReadDir("/sys/block")
	if err != nil {
		return "", fmt.Errorf("error reading /sys/block: %w", err)
	}
	...
	for _, entry := range sysBlock {
		...
		for _, slave := range slaves {
			isESP, err := memberIsESP(slave.Name(), probeOptions...)
			if err != nil || !isESP {
				allESP = false
	...
	case 1:
		return found[0], nil
	default:
		return "", fmt.Errorf("found %d md arrays whose members are EFI System Partitions (%s); "+
...
func memberIsESP(member string, probeOptions ...blkid.ProbeOption) (bool, error) {
	...
	info, err := blkid.ProbePath(filepath.Join("/dev", parent), probeOptions...)
	if err != nil {
		return false, err
	}

	for _, part := range info.Parts {
		if filepath.Base(partitioning.DevName(filepath.Join("/dev", parent), uint(part.PartitionIndex))) != member {
			continue
		}

		return part.PartitionType != nil &&
